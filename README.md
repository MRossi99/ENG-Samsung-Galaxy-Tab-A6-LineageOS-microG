...
<br>
LineageOS does not support Samsung galaxy tab A6 officially. If you're trying to use BitGApps, NikGApps or MineTheGApps for Google services, you will encounter a lot of tricky issues. <br>
...
**1. Inject TWRP into the device**
  - Go to Settings, System and tap on Build Number many times to activate **developer mode**.
  ...
  - Open **Odin** on your PC. Connect the tablet. You may see a small blue rectangle `[ID:COM]`, if you see it, it means you're doing it correctly.
  - Click on **AP** and select the TWRP file (`.tar`).
  - Go to Odin **Options** below and **remove the check** to "Auto Reboot". **(IMPORTANT)**
  - Press **Start** and when you see the green box **PASS** disconnect the cable.
  - Now press `Volume down + Home + Power` simultaneously. When the screen become dark move your fingers to `Volume up + Home + Power` and release when you see the TWRP menu. <br>
    >In the case you fail the last step, don't worry. You can do it again, just enter into download mode, open odin and put the .tar file on AP and inject.<br>
    
**2. Cleaning the device (Wipe)**
  - You are now inside the TWRP, *swipe* and enter in the main menu.
...
**3. Install LineageOS and MicroG**
  - Connect the tablet to your PC with an USB cable (TWRP will recognize it as an external memory unit).
    > It's important to highlight that not all cables can transfer Data, some cables just recharge devices and nothing more, so pay attention and choose the right cable that can do both.
  - Copy the file `.zip` of **LineageOS** and **microG** inside the tablet memory.
    > It is also possible to transfer files through a MicroSD card. To do that just copy all files there and put them inside your tablet.
  - On your tablet, go to **Install**
  - Select **LineageOS** `.zip` and install.
    > If you don't see any files, just reboot your device, go to TWRP settings and **Recovery**. The device will reboot, but it will take you back to the TWRP menu.
...
*Now we need to download the apps, first of all download F-Droid (Enable download from external apps).*
  - Open the browser (Purple icon with a white star inside), search for **F-Droid**. It must be [this site](https://f-droid.org/it/).
    > **F-Droid** is useful if you need a bunch of open-source apps. Go check it out!
...
> [!NOTE]
> For most Google applications everything will work without any problems, but Youtube App is one of the few that needs additional steps. [Here](https://revanced.app/) is the link if you are interested. <br>

Installation is finished, now enjoy your new device with infinite open source possibilities! <br>

**If you want to back to the last Android version or your device doesn't work**<br>
You can revert everything:<br>
- Go [here](https://samfw.com/firmware/SM-T585) and download the last version of the firmware (select your device language).
...
>[!TIP]
>This action will revert everything, and it can be useful if TRWP crashes or gives you irreparable errors.
